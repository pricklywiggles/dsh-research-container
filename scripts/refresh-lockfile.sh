...
# The throwaway image is a full dsh-research-container generation, about 4.8 GB, and nothing
...
cleanup() {
  container image delete dsh-research-container-lockrefresh >/dev/null 2>&1 || true
...
container build --dns 1.1.1.1 --no-cache \
  --build-arg REFRESH_LOCK=1 --build-arg UID="$USER_UID" -t dsh-research-container-lockrefresh . >/dev/null 2>&1 || {
    echo "build failed; run ./run.sh to see the error" >&2; exit 1; }

echo "==> Extracting the resolved lockfile"
container run --rm dsh-research-container-lockrefresh \
