...
def load_fields_yaml(fields_path):
    """Parse fields.yaml in the two shapes the research skills emit:
    ...
    The categorized shape came first; the outline phase also emits the flat
    shape (field name -> attribute dict, no categories), observed in a live
    run on 2026-09-01, where the validator refused it and the completion gate
    silently never functioned. Flat fields all land in one "fields" category.
    Anything else still fails loudly instead of passing with zero fields.
    ...
    if not isinstance(fn, dict):
        print(f"[ERROR] fields.yaml must use the `fields: {{<category>: [{{name, ...}}]}}` or `fields: {{<name>: {{...}}}}` shape; got {type(fn).__name__ if fn is not None else 'None'}.")
        sys.exit(1)

    values = [v for k, v in fn.items() if k not in _SKIP_KEYS]
    flat = values and all(isinstance(v, dict) and "name" not in v for v in values)

    if flat:
        for fname, attrs in fn.items():
            if fname in _SKIP_KEYS:
                continue
            defs.append((str(fname), "fields", attrs.get("required", None)))
    else:
...
def main():
    ...
    if not json_files:
        # Exit 1, not 0: the run skill gates completion on this script, and a
        # vacuous pass would declare a run with zero output complete.
        print("[ERROR] No JSON files found; nothing was validated")
        sys.exit(1)
    results = []
    missing = 0
    ...
    if missing or not results or passed < len(results):
        sys.exit(1)
